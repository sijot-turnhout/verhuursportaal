...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('inventories', static function (Blueprint $table): void {
            $table->id()->comment('Unieke identificatienummer voor elk materieel stuk in de inventaris.');
            $table->boolean('loanable')->default(true)->comment('Indicator dat het inventaris artikel uitleenbaar is');
            $table->string('state')->comment('De huidige staat van de artikelen in de inventaris');
            ...
            $table->foreignIdFor(InventoryCategory::class)->nullable()->comment('Verwijzig naar de categorieen tabel voor de categoirie')->references('id')->on('inventory_categories')->nullOnDelete();
            $table->text('description')->comment('Beschrijving van het materieel. (bijv. afmetingen, capaciteit)')->nullable();
            $table->integer('amount')->comment('Totaal aantal van dit type materieel in de inventaris.')->default('1');
            $table->integer('in_stock')->comment('Aantal beschikbaar voor uitleen/gebruik op dit moment.')->nullable();
            $table->timestamps();
        });

        Schema::create('inventory_inspections', static function (Blueprint $table): void {
            ...
            $table->foreignIdFor(Inventory::class)->comment('Verwijzing naar het gecontroleerde materieel in de materieel inventaris')->references('id')->on('inventories')->cascadeOnDelete();
